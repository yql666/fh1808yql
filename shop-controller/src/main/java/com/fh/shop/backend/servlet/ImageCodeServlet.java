...
public class ImageCodeServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//request.getSession().setAttribute(SystemContent.IMAGE_IMAGECODE, scode);
		String uuid = UUID.randomUUID().toString().replace("-", "").toUpperCase();
		String fh_id = CookieUtil.requestCookie(request, SystemContent.COOKIE_KEY);
		if(StringUtils.isEmpty(fh_id)){
			CookieUtil.addCookie(response,SystemContent.COOKIE_KEY,uuid,SystemContent.LOGIN_URL,-1);
			RedisUtil.set(SystemContent.LOGIN_CODE+uuid,scode);
			RedisUtil.expire(SystemContent.LOGIN_CODE+uuid,2*60);
		}else {
			RedisUtil.set(SystemContent.LOGIN_CODE+uuid,scode);
			RedisUtil.expire(SystemContent.LOGIN_CODE+uuid,2*60);
		}
